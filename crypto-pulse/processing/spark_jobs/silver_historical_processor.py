"""
silver_historical_processor.py — Silver Layer: Historical OHLCV Processor

Reads structured OHLCV data from the Bronze layer (bronze/historical),
which was written by the historical_loader.py Spark batch job.

This script:
  1. Reads from bronze/historical Delta table.
  2. Detects incremental new rows (open_time > last Silver open_time).
  3. Applies DQ: drops nulls, zero prices, and duplicates.
  4. Adds partition columns: year / month / day.
  5. Writes cleaned data to silver/historical in Delta format.
  6. Runs OPTIMIZE for compaction.

Usage:
  spark-submit processing/spark_jobs/silver_historical_processor.py
"""
...
logger = logging.getLogger("SilverHistoricalProcessor")
...
def create_spark_session(config):
    ...
    logger.info("Starting Spark session for Silver Historical processing...")
    sa = config["storage_account"]

    spark = (
        SparkSession.builder
        .appName("SilverHistoricalProcessor")
